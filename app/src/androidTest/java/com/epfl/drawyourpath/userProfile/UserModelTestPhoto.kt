...
class UserModelTestPhoto {
    ...
    fun createCorrectUserWithNullPhoto(){
        val user: UserModel = UserModel(userId,email,username,firstname,surname,dateOfBirth,distanceGoal, timeGoal,nbOfPaths, null, database)
    ...
    fun createCorrectUserWithPhoto(){
        val user: UserModel = UserModel(userId,email,username,firstname,surname,dateOfBirth,distanceGoal, timeGoal,nbOfPaths, photoProfile, database)
        Assert.assertEquals(user.getUserId(), userId)
        Assert.assertEquals(user.getUsername(), username)
        Assert.assertEquals(user.getEmailAddress(), email)
        Assert.assertEquals(user.getFirstname(), firstname)
